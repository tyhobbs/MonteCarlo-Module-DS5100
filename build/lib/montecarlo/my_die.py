...
class Die:
    ...
    def __init__(self):
        ...
        Weights=1
        if any(not isinstance(x, (int, float)) for x in np.ravel(dict1)):
            raise TypeError("Array elements must be numeric")
        if any(not isinstance(x, (int, int)) for x in np.ravel(dict1)):
            raise TypeError("Array elements must be numeric")
        if any(not isinstance(x, (int, str)) for x in np.ravel(dict1)):
            raise TypeError("Array elements must be strings")
        ...
        for column in array1.T:
            if val not in Faces:
                raise ValueError("Array's Value's must be distinct")
    ...
    def new_weight(self):
        ...
        """
        Faces={'Face_1':1,'Face_2':1,'Face_3':1,'Face_4':1,'Face_5':1,'Face_6':1}
        New_Faces={'Face_1':1,'Face_2':1,'Face_3':1,'Face_4':1,'Face_5':1,'Face_6':1}
        new_Weight=1
        ...
        if any(not isinstance(x, (int, float)) for x in np.ravel(array1)):
            raise TypeError("Array elements must be numeric")
        if any(not isinstance(x, (int, int)) for x in np.ravel(array1)):
            raise TypeError("Array elements must be numeric")
        if not np.issubdtype(array1.dtype, np.number):
            raise TypeError("Array elements must be numeric")
